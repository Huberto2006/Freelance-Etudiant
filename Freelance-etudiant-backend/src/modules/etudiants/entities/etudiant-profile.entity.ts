...
/**
 * Profil specifique a un utilisateur ayant le role ETUDIANT.
 * Relation 1-1 avec Utilisateur (partage la meme cle primaire).
 */
...
export class EtudiantProfile {
  ...
  @Column({ name: 'niveau_etude', type: 'varchar', length: 50, nullable: true })
  niveauEtude?: string;
  ...
  @Column({ type: 'text', array: true, default: () => "'{}'", name: 'portfolio_urls' })
  portfolioUrls: string[];

  /**
   * Score de reputation calcule (module 5.2 du cahier des charges).
   * Recalcule par le module ReputationService a chaque nouvelle evaluation.
   */
  @Column({
  ...
  @Column({ name: 'nombre_missions_terminees', type: 'int', default: 0 })
